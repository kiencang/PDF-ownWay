...
export class SearchBarComponent {
  ...
  async onSearch(query: string) {
    ...
    try {
      const result = await this.aiService.translateSearchQuery(query, this.translationState.selectedModel());
      ...
      if (parsedError.includes('429') || parsedError.toLowerCase().includes('quota')) {
        ...
        this.toastService.show('error', 'Lỗi: Yêu cầu dịch từ khóa bị từ chối do vi phạm chính sách an toàn của AI.');
      }
