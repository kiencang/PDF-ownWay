...
import { TranslationMode } from './translation.state';
...
export class App {
  ...
  public translationState = inject(TranslationState);
  ...
  constructor() {
    ...
    if (typeof localStorage !== 'undefined') {
      ...
      const savedModel = localStorage.getItem('sila_pdf_translator_custom_model');
      if (savedModel) {
        this.translationState.selectedModel.set(savedModel);
      }
      const savedKey = localStorage.getItem('ownway_meta_api_key') || localStorage.getItem('sila_pdf_translator_user_api_key');
      if (savedKey) {
  ...
  onSettingsSave(settings: {mode: TranslationMode}) {
    if (settings) {
  ...
  saveApiKey(config: { apiKey: string, model: string }) {
    const keyVal = config.apiKey;
    const modelVal = config.model;
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem('ownway_meta_api_key', keyVal);
      localStorage.setItem('sila_pdf_translator_user_api_key', keyVal);
      localStorage.setItem('sila_pdf_translator_custom_model', modelVal);
    }
    this.translationState.userApiKey.set(keyVal);
    this.translationState.selectedModel.set(modelVal);
    this.showApiKeyModal.set(false);
    if (keyVal) {
      this.showToast('success', 'Đã lưu cấu hình Meta AI thành công!');
    } else {
      this.showToast('info', 'Bạn chưa nhập API Key, nhưng đã lưu cấu hình model.');
    }
  }

  clearApiKey() {
    if (typeof localStorage !== 'undefined') {
      localStorage.removeItem('ownway_meta_api_key');
      localStorage.removeItem('sila_pdf_translator_user_api_key');
      localStorage.removeItem('sila_pdf_translator_custom_model');
    }
    this.translationState.userApiKey.set('');
    this.translationState.selectedModel.set('muse-spark-1.2');
    this.tempApiKey.set('');
    this.showApiKeyModal.set(false);
    if (this.resetBtn && this.resetBtn.nativeElement) {
  ...
  downloadHtml() {
    ...
    if (htmlText) {
      ...
      const modelLabel = (this.translationState.selectedModel() || 'muse-spark-1.2').trim();
