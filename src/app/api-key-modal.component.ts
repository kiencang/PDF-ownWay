      ...
      <div class="bg-white rounded-2xl shadow-xl max-w-2xl w-full p-0 animate-in zoom-in-95 duration-200 overflow-hidden" (click)="$event.stopPropagation()">
        <div class="p-5 border-b border-slate-100 flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <h3 id="api-key-title" class="text-lg font-bold text-slate-900">Cấu hình Meta AI API Key</h3>
          </div>
        ...
        <div class="p-6">
          ...
          <div class="flex items-center justify-between gap-2 text-sm mt-3 bg-slate-50/50 px-3 py-2 rounded-xl border border-slate-100">
            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold"
              ...
              {{ hasUserApiKey ? 'Đang dùng cấu hình Meta AI của bạn' : 'Bạn chưa cấu hình Meta AI' }}
            </span>
            <span class="text-slate-300">|</span>
            <a href="https://dev.meta.ai/api-keys/" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1 text-xs text-indigo-600 hover:text-indigo-800 hover:underline font-medium transition-colors cursor-pointer">
              <lucide-icon [img]="ExternalLink" class="w-3.5 h-3.5" aria-hidden="true"></lucide-icon>
              <span>Nơi lấy API Key Meta AI</span>
            </a>
          </div>
          ...
          <div class="mt-5 space-y-4">
            <div>
              ...
              <input 
                ...
                type="text"
                [(ngModel)]="tempModel"
                class="block w-full px-3 py-2.5 border border-slate-300 rounded-xl bg-slate-50 text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm font-mono tracking-wide"
                placeholder="VD: muse-spark-1.2"
              >
            </div>
            
            <div>
              <label for="meta-custom-api-key" class="text-xs uppercase text-slate-500 font-bold tracking-wider block mb-2">META AI API KEY CÁ NHÂN</label>
              <div class="relative">
                <input 
                  id="meta-custom-api-key"
                  ...
                  placeholder="Nhập Meta API Key của bạn..."
                >
                <button 
                  type="button"
...
export class ApiKeyModalComponent implements OnInit {
  ...
  @Input() initialApiKey = '';
  @Input() initialModel = 'muse-spark-1.2';
  @Input() hasUserApiKey = false;
  
  @Output() save = new EventEmitter<{apiKey: string, model: string}>();
  ...
  ngOnInit() {
    ...
    this.tempModel = this.initialModel || 'muse-spark-1.2';
  }
  ...
  onSaveApiKey() {
    this.save.emit({
      ...
      model: this.tempModel.trim() || 'muse-spark-1.2'
    });
  }

  onClearApiKey() {
    this.tempApiKey = '';
    this.tempModel = 'muse-spark-1.2';
    this.clear.emit();
  }
}
