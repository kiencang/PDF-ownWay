...
export class PdfService {
  ...
  /**
   * Trích xuất văn bản chữ (Text content) từ dải trang PDF
   */
  async extractTextFromPDF(file: File, startPage: number, endPage: number): Promise<string> {
    try {
      ...
      const actualEnd = Math.max(1, Math.min(endPage, totalPages));
      ...
      console.warn('Lỗi khi trích xuất text từ PDF:', e);
      return '';
    }
  }
